...
describe('useForm', () => {
    describe('register', () => {
        ...
        test('return right default value', () => {
            const { result } = renderHook(() => useForm());
            const { ref, name, value, type, onChange, defaultValue, defaultChecked } = result.current.register(undefined);
        ...
        test('return defaultChecked as defaultValue for radio', () => {
            const { result } = renderHook(() => useForm());
            expect(result.current.register('a.b', {
                type: 'radio',
                defaultChecked: true,
            }).defaultChecked).toEqual(true);
            expect(result.current.register('a.b', {
                type: 'radio',
                defaultChecked: false,
            }).defaultChecked).toEqual(false);
        });
